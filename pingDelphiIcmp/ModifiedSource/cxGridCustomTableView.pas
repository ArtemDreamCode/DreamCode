...
{$ENDIF}
  ...
  cxTextEdit, dxIncrementalFiltering;
...
const
  ...
  cxGridFindPanelDefaultMRUItemsListDropDownCount = 8;
  cxGridFindPanelDefaultMRUItemsListCount = 0;
...
type
  ...
  TcxGridFindPanelComboBoxInnerEdit = class(TcxCustomComboBoxInnerEdit)
  private
    function GetContainer: TcxGridFindPanelMRUEdit;

    procedure WMGetDlgCode(var Message: TWMGetDlgCode); message WM_GETDLGCODE;
  protected
    property Container: TcxGridFindPanelMRUEdit read GetContainer;
  end;

  TcxGridFindPanelMRUEdit = class(TcxCustomMRUEdit)
  private
    FFindPanel: TcxGridFindPanel;
    FNeedRecalculated: Boolean;
  protected
    ...
    property NeedRecalculate: Boolean read FNeedRecalculated write FNeedRecalculated;
  public
    constructor Create(AFindPanel: TcxGridFindPanel); reintroduce; virtual;
    procedure AddItem(const Value: string); override;
    procedure Hide;
    procedure SetFocus; override;
    procedure Show(AParent: TWinControl; const ABounds: TRect);
  end;
  ...
  protected
    ...
    procedure DrawItemText(ACanvas: TcxCanvas; AIndex: Integer; const ARect: TRect); override;
  ...
  TcxGridFilterPopupIncrementalFilteringHelper = class(TdxIncrementalFilteringFilterPopupHelper)
  private
    FPopup: TcxGridFilterPopup;
  protected
  ...
  public
    constructor Create(AOwner: TcxGridFilterPopup);
  end;

  TcxGridFilterPopup = class(TcxCustomGridPopup)
  private
    FButton: TcxButton;
    FCheckedFilterValueIndexes: TcxGridIndexes;
    FIncrementalFilteringHelper: TcxGridFilterPopupIncrementalFilteringHelper;
    FFilterChangedByCheck: Boolean;
    FFilterItems: TStringList;
    FItem: TcxCustomGridTableItem;
    FListBox: TcxGridFilterPopupListBox;
    FListBoxItemByFilterValueIndexes: TDictionary<Integer, Integer>;
    FListBoxItems: TStringList;
    FValueList: TcxGridFilterValueList;
    procedure FocusSearchControl;
    function GetFilter: TcxDataFilterCriteria;
    function GetFilterValueByListItemIndex(AItemIndex: Integer): TcxFilterValueItem;
    function GetFilterValueIndexByListItemIndex(AItemIndex: Integer): Integer;
    function GetGridView: TcxCustomGridTableView;
    function GetListBoxItemIndexByFilterValueIndex(AIndex: Integer): Integer;
    ...
    function IsSearchInfoPanelVisible: Boolean;
  ...
  protected
    ...
    property FilterValueIndexByListItemIndex[Index: Integer]: Integer read GetFilterValueIndexByListItemIndex;
    ...
    property ListBoxItemIndexByFilterValueIndex[Index: Integer]: Integer read GetListBoxItemIndexByFilterValueIndex;
...
//    property Owner: IcxGridFilterPopupOwner read GetOwnerValue write SetOwnerValue;
  ...
  protected
    ...
    function CanMoveItems(AItems: TList): Boolean; virtual;
  ...
  protected
    ...
    procedure OnBaseMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer); virtual;
    procedure OnBaseMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer); virtual;
    procedure OnBaseMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer); virtual;
  ...
  private
    ...
    function GetController: TcxCustomGridTableController;
    function GetEdit: TcxGridFindPanelMRUEdit;
  protected
    function DoCalculateHeight: Integer; override;
    function DoCalculateWidth: Integer; override;
    procedure GetViewParams(var AParams: TcxViewParams); override;
    ...
    property Controller: TcxCustomGridTableController read GetController;
    property Edit: TcxGridFindPanelMRUEdit read GetEdit;
  public
  ...
  private
    ...
    function GetItem(Index: Integer): TcxGridFilterMRUItem; inline;
    function GetVisibleCount: Integer; inline;
    function GetVisibleItem(Index: Integer): TcxGridFilterMRUItem; inline;
  protected
  ...
  TcxFindPanelFocusedItem = (fpfiNone, fpfiCloseButton, fpfiFindEdit, fpfiFindButton, fpfiClearButton);

  TcxGridFindPanel = class
  private
    FController: TcxCustomGridTableController;
    FDelayedFilteringTimer: TcxTimer;
    FEdit: TcxGridFindPanelMRUEdit;
    FFocusedItem: TcxFindPanelFocusedItem;
    FFreeNotificator: TcxFreeNotificator;
    FIsEditUpdatingCount: Integer;
    FMRUItems: TStrings;
    FUseExtendedSyntax: Boolean;
    FVisible: Boolean;

    function GetFindFilterText: string;
    function GetOptions: TcxGridFindPanelOptions;
    function GetGridView: TcxCustomGridTableView;
    function GetMRUItems: TStrings;
    procedure SetEdit(AEdit: TcxGridFindPanelMRUEdit);
  ...
  protected
    procedure AdjustEditValueByFindFilter; virtual;
    procedure AddFindFilterTextInEditDropDownList; virtual;
    procedure AdjustFindFilterByEditValue; virtual;
    procedure AdjustFocusedRecord; virtual;
    procedure ApplyFindFilter(const AText: string); virtual;
    function CanHide: Boolean; virtual;
    function CanShow: Boolean; virtual;
    procedure Changed(AChangeKind: TcxGridViewChangeKind); virtual;
    procedure ClearFindFilter; virtual;
    procedure DoButtonsExecute; virtual;
    procedure DoEditLookAndFeelUpdate(AEdit: TcxGridFindPanelMRUEdit); virtual;
    procedure DoEditStylesUpdate(AEdit: TcxGridFindPanelMRUEdit); virtual;
    procedure FocusFirstGridRow; virtual;
    procedure FocusedNextItem; virtual;
    procedure FocusedPreviewItem; virtual;
    procedure UpdateEdit; virtual;
    procedure UpdateStyles(AEdit: TcxGridFindPanelMRUEdit); virtual;
    procedure UpdateTextHint; virtual;
    procedure ValidateFocusedItem(AIsFocusedNext: Boolean = True); virtual;

    //edit key handlers
    procedure AssignOverridableEditEvents(AEdit: TcxGridFindPanelMRUEdit); virtual;
    ...
    function CreateEdit: TcxGridFindPanelMRUEdit; virtual;
    procedure DestroyEdit; virtual;

    property FindFilterText: string read GetFindFilterText;
  public
    constructor Create(AController: TcxCustomGridTableController); virtual;
    destructor Destroy; override;
    ...
    function IsEditUpdating: Boolean; virtual;
    procedure KeyDown(var Key: Word; Shift: TShiftState); virtual;
    procedure KeyPress(var Key: Char); virtual;
    procedure KeyUp(var Key: Word; Shift: TShiftState); virtual;
    procedure LookAndFeelChanged; virtual;
    procedure Show; virtual;
    procedure StartDelayingFiltering; virtual;
    procedure StylesChanged; virtual;

    property Controller: TcxCustomGridTableController read FController;
    property Edit: TcxGridFindPanelMRUEdit read FEdit;
    ...
    property Options: TcxGridFindPanelOptions read GetOptions;
  ...
  private
    ...
    FClearFindFilterTextOnClose: Boolean;
    FDisplayMode: TcxFindPanelDisplayMode;
    FFocusViewOnApplyFilter: Boolean;
    ...
    function DefaultInfoText: string;
  ...
  public
    ...
    property MRUItems: TStrings read GetMRUItems write SetMRUItems;
  published
    property ApplyInputDelay: Integer read FApplyInputDelay write SetApplyInputDelay default 1000;
    property ClearFindFilterTextOnClose: Boolean read FClearFindFilterTextOnClose write SetClearFindFilterTextOnClose default True;
    property DisplayMode: TcxFindPanelDisplayMode read FDisplayMode write SetDisplayMode default fpdmNever;
    property FocusViewOnApplyFilter: Boolean read FFocusViewOnApplyFilter write SetFocusViewOnApplyFilter default False;
    property HighlightSearchResults: Boolean read FHighlightSearchResults write SetHighlightSearchResults default True;
    property InfoText: string read GetInfoText write SetInfoText stored IsInfoTextStored;
    property MRUItemsListCount: Integer read FMRUItemsListCount write SetMRUItemsListCount default cxGridFindPanelDefaultMRUItemsListCount;
    property MRUItemsListDropDownCount: Integer read FMRUItemsListDropDownCount write SetMRUItemsListDropDownCount default cxGridFindPanelDefaultMRUItemsListDropDownCount;
    property Position: TcxFindPanelPosition read FPosition write SetPosition default fppTop;
    property ShowClearButton: Boolean read FShowClearButton write SetShowClearButton default True;
    property ShowCloseButton: Boolean read FShowCloseButton write SetShowCloseButton default True;
    property ShowFindButton: Boolean read FShowFindButton write SetShowFindButton default True;
    property UseDelayedFind: Boolean read FUseDelayedFind write SetUseDelayedFind default True;
...
uses
  ...
  cxLibraryConsts, cxGeometry, cxFormats, cxVariants, cxDateUtils, cxDrawTextUtils,
...
{ TcxGridFindPanelComboBoxInnerEdit }

function TcxGridFindPanelComboBoxInnerEdit.GetContainer: TcxGridFindPanelMRUEdit;
begin
  Result := TcxGridFindPanelMRUEdit(inherited Container);
end;

procedure TcxGridFindPanelComboBoxInnerEdit.WMGetDlgCode(var Message: TWMGetDlgCode);
begin
  inherited;
...
constructor TcxGridFindPanelMRUEdit.Create(AFindPanel: TcxGridFindPanel);
begin
  inherited Create(AFindPanel.GridView);
  ...
  Visible := False;
  FNeedRecalculated := True;
  Properties.ShowEllipsis := False;
end;

procedure TcxGridFindPanelMRUEdit.AddItem(const Value: string);
begin
  inherited AddItem(Value);
  if not FindPanel.IsEditUpdating then
    FindPanel.MRUItems.Assign(Properties.LookupItems);
end;

procedure TcxGridFindPanelMRUEdit.Hide;
begin
...
procedure TcxGridFindPanelMRUEdit.SetFocus;
begin
  if HandleAllocated and IsWindowVisible(Handle) then
    inherited SetFocus;
end;

procedure TcxGridFindPanelMRUEdit.Show(AParent: TWinControl; const ABounds: TRect);
begin
  Parent := AParent;
  CheckHandle;
...
procedure TcxGridFindPanelMRUEdit.FocusChanged;
begin
  if csDestroying in Parent.ComponentState then
    Exit;
  inherited FocusChanged;
  if Focused or (FindPanel.FocusedItem = fpfiFindEdit) then
  begin
    if Focused then
      FindPanel.FocusedItem := fpfiFindEdit
    else
      FindPanel.FocusedItem := fpfiNone;
    TcxControlAccess(FindPanel.GridView.Site).FocusChanged;
  end;
end;

function TcxGridFindPanelMRUEdit.GetInnerEditClass: TControlClass;
begin
  Result := TcxGridFindPanelComboBoxInnerEdit;
end;

procedure TcxGridFindPanelMRUEdit.SetInternalEditValue(const Value: Variant);
begin
  inherited;
  FindPanel.AdjustFindFilterByEditValue;
end;
...
begin
  Result := inherited MayFocus and
    ((Site <> nil) and not Site.IsFocused or
     not IsEditing or
     not GridView.OptionsBehavior.AlwaysShowEditor and TcxCustomEditAccess(FEditingController.Edit).InternalValidateEdit);
end;
...
begin
  Result := False;
  if EditingController.IsErrorOnEditExit then
    Exit;
...
begin
  Result := (ARecord <> nil) and (GridView.Control <> nil);
  if Result then
  begin
...
begin
  ...
  try
    ...
    if (AIndex < 0) or not DataController.IsGridMode and (AIndex = InternalTopRecordIndex) and
...
begin
  if IsPixelBasedScrollDataPos then
    if (ViewInfo.VisibleRecordCount > 0) and (InternalTopRecordIndex <> -1) then
      if GridView.IsEqualScrollSizeRecords then
        Result := InternalTopRecordIndex * ViewInfo.GetEqualHeightRecordScrollSize - PixelScrollRecordOffset
      else
        Result := ViewData.Records[InternalTopRecordIndex].PixelScrollPosition - PixelScrollRecordOffset
    else
      Result := 0
  else
    Result := ScrollBarOffsetBegin + InternalTopRecordIndex;
end;
...
begin
  inherited;
  if FindPanel.Visible and FindPanel.IsFocused and
    (not GridView.IsControlFocused or not FindPanel.Edit.IsFocused) then
    FindPanel.FocusedItem := fpfiNone;
  InvalidateSelection;
end;
...
procedure TcxCustomGridTableController.OnBaseMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
begin

end;

procedure TcxCustomGridTableController.OnBaseMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
begin

end;

procedure TcxCustomGridTableController.OnBaseMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
begin
  ...
  case Key of
    ...
    VK_RETURN, VK_TAB:
      if (Key = VK_RETURN) and GridView.OptionsBehavior.GoToNextCellOnEnter or
        (Key = VK_TAB) and (GridView.OptionsBehavior.FocusCellOnTab or FindPanel.Visible) then
      begin
        ARemoveFocus := False;
        if (Key = VK_TAB) and FindPanel.Visible and not GridView.OptionsBehavior.FocusCellOnTab then
          ARemoveFocus := not EditingController.IsEditing
        else
          if Shift + [ssShift] = [ssShift] then
            if FocusNextCell(Shift = [], False, True, False, MultiSelect) or (Key = VK_RETURN) then
              Key := 0
            else
              ARemoveFocus := not EditingController.IsEditing
          else
...
destructor TcxGridFindPanel.Destroy;
begin
  DestroyMRUItems;
  DestroyTimer;
  SetEdit(nil);
...
procedure TcxGridFindPanel.ClearButtonExecute;
begin
  ClearFindFilter;
  Edit.SetFocus;
end;

procedure TcxGridFindPanel.CloseButtonExecute;
begin
  Hide;
end;

procedure TcxGridFindPanel.FindButtonExecute;
begin
  AdjustFindFilterByEditValue;
  AddFindFilterTextInEditDropDownList;
  Edit.SetFocus;
  Edit.SelectAll;
end;

procedure TcxGridFindPanel.FindFilterChanged;
begin
  DestroyTimer;
  if Visible then
    AdjustEditValueByFindFilter;
  UseExtendedSyntax := GridView.DataController.UseExtendedFindFilterTextSyntax;
  if Options.FocusViewOnApplyFilter then
    FocusFirstGridRow;
end;

procedure TcxGridFindPanel.Hide;
begin
  if CanHide then
    Visible := False;
end;

function TcxGridFindPanel.IsFocused: Boolean;
begin
  Result := FocusedItem <> fpfiNone;
end;

function TcxGridFindPanel.IsEditUpdating: Boolean;
begin
  Result := FIsEditUpdatingCount > 0;
end;

procedure TcxGridFindPanel.KeyDown(var Key: Word; Shift: TShiftState);
begin
  case Key of
    VK_RETURN, VK_SPACE:
      begin
  ...
  if (Key <> 0)and Options.UseDelayedFind then
    StartDelayingFiltering;
end;

procedure TcxGridFindPanel.KeyPress(var Key: Char);
begin
// do nothing
end;

procedure TcxGridFindPanel.KeyUp(var Key: Word; Shift: TShiftState);
begin
//do nothing
end;

procedure TcxGridFindPanel.LookAndFeelChanged;
begin
  UpdateStyles(Edit);
end;

procedure TcxGridFindPanel.Show;
begin
  if CanShow then
    Visible := True;
end;

procedure TcxGridFindPanel.StartDelayingFiltering;
begin
  DestroyTimer;
  CreateTimer;
end;

procedure TcxGridFindPanel.StylesChanged;
begin
  UpdateStyles(Edit);
end;

procedure TcxGridFindPanel.AdjustEditValueByFindFilter;
var
  AEditValue: TcxEditValue;
begin
  if FindFilterText = '' then
    AEditValue := Null
  else
    AEditValue := FindFilterText;
  Edit.EditValue := AEditValue;
end;

procedure TcxGridFindPanel.AddFindFilterTextInEditDropDownList;
begin
  Edit.AddItem(FindFilterText);
end;

procedure TcxGridFindPanel.AdjustFindFilterByEditValue;
var
  AFindFilter: string;
begin
  if not Visible then
  ...
  if VarIsNull(Edit.EditingValue) then
    ...
    AFindFilter := Edit.EditingValue;
  ApplyFindFilter(AFindFilter);
end;

procedure TcxGridFindPanel.AdjustFocusedRecord;
begin
  if Controller.FocusedRecord = nil then
    Controller.FocusedRecordIndex := 0;
end;

procedure TcxGridFindPanel.ApplyFindFilter(const AText: string);
begin
  Controller.ApplyFindFilterText(AText);
end;

function TcxGridFindPanel.CanHide;
begin
  Result := Options.DisplayMode in [fpdmNever, fpdmManual];
end;

function TcxGridFindPanel.CanShow;
begin
  Result := Options.DisplayMode in [fpdmManual, fpdmAlways];
end;

procedure TcxGridFindPanel.Changed(AChangeKind: TcxGridViewChangeKind);
begin
  Options.Changed(AChangeKind);
end;

procedure TcxGridFindPanel.ClearFindFilter;
begin
  if FindFilterText = '' then
  begin
    DestroyTimer;
...
procedure TcxGridFindPanel.DoButtonsExecute;
begin
  case FocusedItem of
    fpfiCloseButton:
      CloseButtonExecute;
...
procedure TcxGridFindPanel.DoEditLookAndFeelUpdate(AEdit: TcxGridFindPanelMRUEdit);
begin
  AEdit.Style.LookAndFeel.MasterLookAndFeel := GridView.LookAndFeel;
  AEdit.NeedRecalculate := True;
end;

procedure TcxGridFindPanel.DoEditStylesUpdate(AEdit: TcxGridFindPanelMRUEdit);
var
  AParams, AContentParams: TcxViewParams;
begin
  if GridView.Styles.FindPanel <> nil then
  begin
    GridView.Styles.GetViewParams(vsFindPanel, nil, nil, AParams);
    GridView.Styles.GetViewParams(vsContent, nil, nil, AContentParams);
    AParams.Color := AContentParams.Color;
    AEdit.Style.Init(AParams);
    AEdit.NeedRecalculate := True;
  end;
end;

procedure TcxGridFindPanel.FocusFirstGridRow;
begin
  Controller.FocusedRecordIndex := 0;
  if Controller.FocusedRecordIndex = 0 then
  begin
...
procedure TcxGridFindPanel.FocusedNextItem;
begin
  case FocusedItem of
...
procedure TcxGridFindPanel.FocusedPreviewItem;
begin
  case FocusedItem of
    fpfiCloseButton:
      FocusedItem := fpfiClearButton;
...
procedure TcxGridFindPanel.UpdateEdit;
var
  AMaxItemCount: Integer;
begin
  if Visible then
  begin
    Inc(FIsEditUpdatingCount);
    try
      if not Edit.Properties.IsChanging then
      begin
        Edit.Properties.LookupItems.Assign(FMRUItems);
        AMaxItemCount := Edit.Properties.MaxItemCount;
        Edit.Properties.MaxItemCount := Options.MRUItemsListCount;
        Edit.Properties.DropDownRows := Options.MRUItemsListDropDownCount;
        if Options.MRUItemsListCount <> Edit.Properties.MaxItemCount then
          Options.MRUItemsListCount := Edit.Properties.MaxItemCount;
        if Options.MRUItemsListDropDownCount <> Edit.Properties.DropDownRows then
          Options.MRUItemsListDropDownCount := Edit.Properties.DropDownRows;
        if AMaxItemCount <> Edit.Properties.MaxItemCount then
          FMRUItems.Assign(Edit.Properties.LookupItems);
      end;
    finally
      Dec(FIsEditUpdatingCount);
    end;
  end;
end;

procedure TcxGridFindPanel.UpdateStyles(AEdit: TcxGridFindPanelMRUEdit);
begin
  DoEditLookAndFeelUpdate(AEdit);
  DoEditStylesUpdate(AEdit);
end;

procedure TcxGridFindPanel.UpdateTextHint;
begin
  Edit.TextHint := Options.InfoText;
end;

procedure TcxGridFindPanel.ValidateFocusedItem(AIsFocusedNext: Boolean = True);
begin
...
procedure TcxGridFindPanel.AssignOverridableEditEvents(AEdit: TcxGridFindPanelMRUEdit);
begin
  AEdit.OnAfterKeyDown := EditAfterKeyDown;
  AEdit.OnKeyDown := EditKeyDown;
end;

procedure TcxGridFindPanel.EditAfterKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  case Key of
    VK_ESCAPE:
      begin
...
procedure TcxGridFindPanel.EditKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  KeyDown(Key, Shift);
end;

function TcxGridFindPanel.CreateEdit: TcxGridFindPanelMRUEdit;
begin
  Result := TcxGridFindPanelMRUEdit.Create(Self);
  Result.Style.LookAndFeel.MasterLookAndFeel := GridView.LookAndFeel;
  AssignOverridableEditEvents(Result);
  UpdateStyles(Result);
end;

procedure TcxGridFindPanel.DestroyEdit;
begin
  FreeAndNil(FEdit);
end;

function TcxGridFindPanel.GetFindFilterText: string;
begin
  Result := Controller.GetFindFilterText;
end;

function TcxGridFindPanel.GetOptions: TcxGridFindPanelOptions;
begin
  Result := GridView.FindPanel;
end;
...
function TcxGridFindPanel.GetMRUItems: TStrings;
begin
  Result := FMRUItems;
end;

procedure TcxGridFindPanel.SetEdit(AEdit: TcxGridFindPanelMRUEdit);
begin
  if FEdit <> AEdit then
  begin
    if FEdit <> nil then
    begin
      if AEdit <> nil then
        AEdit.Properties.LookupItems.Assign(FEdit.Properties.LookupItems);
      FFreeNotificator.RemoveSender(FEdit);
      DestroyEdit;
    end;
    FEdit := AEdit;
    if FEdit <> nil then
    begin
      FFreeNotificator.AddSender(FEdit);
      UpdateEdit;
    end;
  end;
end;

procedure TcxGridFindPanel.SetFocusedItem(AValue: TcxFindPanelFocusedItem);
begin
  ...
  begin
    ...
    if GridView.IsDestroying then
      Exit;
    if FFocusedItem = fpfiFindEdit then
    begin
      Edit.SetFocus;
      Edit.SelectAll;
    end
    else
      if FFocusedItem <> fpfiNone then
        GridView.Site.SetFocus;
    Changed(vcSize);
  end;
end;

procedure TcxGridFindPanel.SetMRUItems(AValue: TStrings);
begin
  if AValue <> FMRUItems then
  begin
    FMRUItems.Assign(AValue);
    UpdateEdit;
  end;
end;
...
begin
  ...
  begin
    ...
    GridView.DataController.UseExtendedFindFilterTextSyntax := FUseExtendedSyntax;
    Changed(vcSize);
  end;
end;

procedure TcxGridFindPanel.SetVisible(AValue: Boolean);
begin
  ...
  begin
    ...
    begin
      FocusedItem := fpfiNone;
      if Options.ClearFindFilterTextOnClose then
        ClearFindFilter;
    ...
    if Visible then
      AdjustEditValueByFindFilter;
    Changed(vcSize);
    GridView.FindPanelVisibilityChanged(FVisible);
  end;
  if FVisible then
    FocusedItem := fpfiFindEdit;
end;

procedure TcxGridFindPanel.CreateTimer;
begin
  FDelayedFilteringTimer := TcxTimer.Create(nil);
  FDelayedFilteringTimer.Interval := Options.ApplyInputDelay;
  FDelayedFilteringTimer.OnTimer := OnDelayingFilteringTimer;
end;

procedure TcxGridFindPanel.DestroyTimer;
begin
  FreeAndNil(FDelayedFilteringTimer);
end;

procedure TcxGridFindPanel.OnDelayingFilteringTimer(Sender: TObject);
begin
  DestroyTimer;
  AdjustFindFilterByEditValue;
end;

procedure TcxGridFindPanel.CreateMRUItems;
begin
  FMRUItems := TStringList.Create;
  TStringList(FMRUItems).OnChange := MRUItemsChangeHandler;
end;

procedure TcxGridFindPanel.DestroyMRUItems;
begin
  FreeAndNil(FMRUItems);
end;

procedure TcxGridFindPanel.FreeNotificationHandler(Sender: TComponent);
begin
  FEdit := nil;
  if not GridView.IsDestroying then
    SetEdit(CreateEdit);
...
procedure TcxGridFindPanel.MRUItemsChangeHandler(Sender: TObject);
begin
  UpdateEdit;
end;
...
constructor TcxGridFindPanelOptions.Create(AGridView: TcxCustomGridView);
begin
  inherited Create(AGridView);
  FMRUItemsListDropDownCount := cxGridFindPanelDefaultMRUItemsListDropDownCount;
  FMRUItemsListCount := cxGridFindPanelDefaultMRUItemsListCount;
  FDisplayMode := fpdmNever;
  FClearFindFilterTextOnClose := True;
...
begin
  ...
  begin
    ...
    ClearFindFilterTextOnClose := AOptions.ClearFindFilterTextOnClose;
...
function TcxGridFindPanelOptions.GetFindPanel: TcxGridFindPanel;
begin
  Result := GridView.Controller.FindPanel;
end;
...
begin
  if FApplyInputDelay <> AValue then
  begin
...
begin
  if FClearFindFilterTextOnClose <> AValue then
  begin
    FClearFindFilterTextOnClose := AValue;
    Changed(vcProperty);
  end;
end;
...
begin
  if AValue <> FFocusViewOnApplyFilter then
  begin
    FFocusViewOnApplyFilter := AValue;
    Changed(vcProperty);
  end;
end;
...
begin
  if AValue < 0 then
    AValue := 0;
  if AValue <> FMRUItemsListCount then
  begin
    FMRUItemsListCount := AValue;
    FindPanel.UpdateEdit;
    if not FindPanel.Visible then
      CheckDropDownListItemCount;
  end;
end;
...
begin
  if AValue < 1 then
    AValue := 1;
  if AValue <> FMRUItemsListDropDownCount then
  begin
...
begin
  if FShowFindButton <> AValue then
  begin
...
begin
  if FUseDelayedFind <> AValue then
  begin
...
function TcxGridFindPanelOptions.DefaultInfoText: string;
begin
...
begin
  ...
  begin
    FInfoText := AValue;
    FInfoTextAssigned := FInfoText <> DefaultInfoText;
    Changed(vcProperty);
  end;
end;
...
begin
  ...
  begin
    AFirstIndex := Max(0, Controller.InternalTopRecordIndex);
...
begin
  ...
  begin
    ...
    if not GridView.Controller.FindPanel.Visible then
...
begin
  Result := GetGridViewInfo.LookAndFeelPainter.ButtonFocusRect(Canvas, ViewInfo.Bounds);
end;
...
begin
  ViewInfo.LookAndFeelPainter.DrawFilterCloseButton(Canvas, ViewInfo.Bounds, ViewInfo.ButtonState);
  DrawFocusRect;
end;
...
begin
  ...
  ViewInfo.LookAndFeelPainter.DrawButton(Canvas, ViewInfo.Bounds, ViewInfo.Text, ViewInfo.ButtonState);
  DrawFocusRect;
end;
...
begin
  ...
  ViewInfo.LookAndFeelPainter.DrawFilterActivateButton(Canvas, ABounds, ViewInfo.ButtonState, ViewInfo.Checked);
end;
...
begin
  ViewInfo.LookAndFeelPainter.DrawFilterDropDownButton(Canvas, ViewInfo.Bounds, ViewInfo.ButtonState, False);
end;
...
begin
  ...
  begin
    if I = 0 then
      ADelta := Max(1, Controller.GetRecordCountInScrollBand(I));
    if I >= ADelta then
    begin
      Inc(ADelta, Controller.GetRecordCountInScrollBand(I));
...
function TcxGridFilterPopupListBox.CanFocus: Boolean;
begin
  Result := inherited CanFocus and not Popup.UseIncrementalFiltering;
end;

procedure TcxGridFilterPopupListBox.FocusSearchControl;
begin
  Popup.FocusSearchControl;
end;
...
procedure TcxGridFilterPopupListBox.Click;
begin
  inherited;
  FocusSearchControl;
end;

procedure TcxGridFilterPopupListBox.DblClick;
begin
  inherited;
  FocusSearchControl;
end;
...
procedure TcxGridFilterPopupListBox.DrawItemContent(ACanvas: TcxCanvas;
  ...
  begin
    ...
    begin
      ...
      ACanvas.Line(Left, Y + 2, Right, Y + 2);
    end;
  end;
...
procedure TcxGridFilterPopupListBox.DrawItemText(ACanvas: TcxCanvas;
  AIndex: Integer; const ARect: TRect);
var
  ASelStart: Integer;
  AText, ASearchText: string;
  R: TRect;
begin
  if Popup.UseIncrementalFiltering and (ifoHighlightSearchText in Popup.IncrementalFilteringOptions) then
  begin
    R := ARect;
    ASearchText := Popup.GetSearchText;
    AText := Items[AIndex];
    ASelStart := Pos(AnsiUpperCase(ASearchText), AnsiUpperCase(AText)) - 1;
    cxTextOut(InnerListBox.Canvas, AText, R, CXTO_LEFT or CXTO_CENTER_VERTICALLY or CXTO_SINGLELINE or CXTO_END_ELLIPSIS,
      ASelStart, Length(ASearchText), ACanvas.Font,
      clHighlight, clHighlightText, 0, 0, 0, ACanvas.Font.Color);
  end
  else
    inherited;
end;
...
{ TcxGridFilterPopupIncrementalFilteringHelper }

constructor TcxGridFilterPopupIncrementalFilteringHelper.Create(
  AOwner: TcxGridFilterPopup);
begin
  inherited Create;
  FPopup := AOwner;
end;

procedure TcxGridFilterPopupIncrementalFilteringHelper.DoKeyDown(var Key: Word;
  Shift: TShiftState);
begin
  inherited DoKeyDown(Key, Shift);
  FPopup.ListBox.KeyDown(Key, Shift);
end;

function TcxGridFilterPopupIncrementalFilteringHelper.GetItemCount: Integer;
begin
  Result := FPopup.ListBox.Count;
end;

function TcxGridFilterPopupIncrementalFilteringHelper.GetItemIndex: Integer;
begin
  Result := FPopup.ListBox.ItemIndex;
end;

function TcxGridFilterPopupIncrementalFilteringHelper.GetLookAndFeelPainter: TcxCustomLookAndFeelPainter;
begin
  Result := FPopup.GridView.LookAndFeelPainter;
end;

function TcxGridFilterPopupIncrementalFilteringHelper.GetMouseWheelHandler: TcxCustomInnerListBox;
begin
  Result := FPopup.ListBox.InnerListBox as TcxCustomInnerListBox;
end;

function TcxGridFilterPopupIncrementalFilteringHelper.GetVisibleItemCount: Integer;
begin
  Result := FPopup.ListBox.VisibleItemCount;
end;

procedure TcxGridFilterPopupIncrementalFilteringHelper.SearchEditValueChanged(
  Sender: TObject);
begin
  FPopup.ResizePopupWindow;
end;

procedure TcxGridFilterPopupIncrementalFilteringHelper.SetItemIndex(
  const Value: Integer);
begin
  FPopup.ListBox.ItemIndex := Value;
end;

procedure TcxGridFilterPopupIncrementalFilteringHelper.UpdateSearchEditPosition;
begin
  SearchEdit.Left := FPopup.BorderWidths[bLeft] + GetSearchEditOffsets.Left;
  if GetSearchText = '' then
    SearchEdit.Top := - SearchEdit.Height
  else
    SearchEdit.Top := FPopup.BorderWidths[bTop] + GetSearchEditOffsets.Top;
end;
...
begin
  ...
  FIncrementalFilteringHelper := TcxGridFilterPopupIncrementalFilteringHelper.Create(Self);
  FIncrementalFilteringHelper.CheckSearchControl(Self);
  FListBoxItemByFilterValueIndexes := TDictionary<Integer, Integer>.Create;
...
begin
  FFilterItems.Free;
  FListBoxItemByFilterValueIndexes.Free;
  FIncrementalFilteringHelper.Free;
  FValueList.Free;
  FListBoxItems.Free;
  inherited;
end;

procedure TcxGridFilterPopup.FocusSearchControl;
begin
...
function TcxGridFilterPopup.GetFilterValueByListItemIndex(
  AItemIndex: Integer): TcxFilterValueItem;
begin
  Result := FListBoxItems.Objects[AItemIndex] as TcxFilterValueItem
end;

function TcxGridFilterPopup.GetFilterValueIndexByListItemIndex(
  AItemIndex: Integer): Integer;
begin
  if UseIncrementalFiltering then
    Result := FValueList.ItemsList.IndexOf(FListBoxItems.Objects[AItemIndex])
  else
    Result := AItemIndex;
end;
...
function TcxGridFilterPopup.GetListBoxItemIndexByFilterValueIndex(AIndex: Integer): Integer;
begin
  if not UseIncrementalFiltering then
    Result := AIndex
  else
    if not FListBoxItemByFilterValueIndexes.TryGetValue(AIndex, Result) then
      Result := -1;
end;

function TcxGridFilterPopup.GetSearchEditOffsets: TRect;
begin
  Result := FIncrementalFilteringHelper.GetSearchEditOffsets;
end;

function TcxGridFilterPopup.GetSearchText: string;
begin
...
function TcxGridFilterPopup.IncrementalFilteringOptions: TcxTextEditIncrementalFilteringOptions;
begin
  Result := Item.Options.FilteringPopupIncrementalFilteringOptions;
end;

function TcxGridFilterPopup.IsSearchInfoPanelVisible: Boolean;
begin
  Result := UseIncrementalFiltering and (FListBoxItems.Count = 0);
end;

function TcxGridFilterPopup.UseIncrementalFiltering: Boolean;
begin
  Result := Item.Options.FilteringPopupIncrementalFiltering;
end;
...
procedure TcxGridFilterPopup.SetItem(AValue: TcxCustomGridTableItem);
begin
  if FItem <> AValue then
  begin
    if FItem <> nil then
      FItem.RemoveFreeNotification(Self);
    FItem := AValue;
    if FItem <> nil then
      FItem.FreeNotification(Self);
  end;
end;
...
begin
  CloseUp;
  ApplyFilterUsingCheckedItems(FCheckedFilterValueIndexes);
...
begin
  ...
  begin
    AFilterValueIndex := FilterValueIndexByListItemIndex[AItemIndex];
    if ListBox.Checked[AItemIndex] then
    begin
      SetLength(FCheckedFilterValueIndexes, Length(FCheckedFilterValueIndexes) + 1);
      FCheckedFilterValueIndexes[High(FCheckedFilterValueIndexes)] := AFilterValueIndex;
    end
    else
    begin
      for I := 0 to High(FCheckedFilterValueIndexes) do
      begin
        if FCheckedFilterValueIndexes[I] = AFilterValueIndex then
        begin
          for J := I to High(FCheckedFilterValueIndexes) - 1 do
            FCheckedFilterValueIndexes[J] := FCheckedFilterValueIndexes[J + 1];
          Break;
        end;
      end;
      SetLength(FCheckedFilterValueIndexes, Length(FCheckedFilterValueIndexes) - 1);
...
var
  ...
  S: string;
begin
  FListBoxItemByFilterValueIndexes.Clear;
  FListBoxItems.BeginUpdate;
  try
    FListBoxItems.Clear;
    for I := 0 to FFilterItems.Count - 1 do
    begin
      S := FFilterItems[I];
      if not UseIncrementalFiltering or
      (GetSearchText = '') or ((FFilterItems.Objects[I] as TcxFilterValueItem).Kind = fviValue) and
        CheckItemFiltered(S) then
      begin
        FListBoxItems.AddObject(S, FFilterItems.Objects[I]);
        FListBoxItemByFilterValueIndexes.Add(I, FListBoxItems.Count - 1);
      end;
    end;
  finally
    FListBoxItems.EndUpdate;
  end;
end;
...
begin
  AItemIndex := FilterValueIndexByListItemIndex[AItemIndex];
  FValueList.ApplyFilter(Item, AItemIndex, nil, True, True);
  if not (FValueList[AItemIndex].Kind in [fviCustom, fviUser]) then
    GridView.Filtering.AddFilterToMRUItems;
end;

function TcxGridFilterPopup.CalculateSize: TSize;
begin
  Result.cx := ListBox.BoundsRect.Right + BorderWidths[bRight];
  if IsButtonVisible then
    Result.cy := Button.BoundsRect.Bottom
  else
    Result.cy := ListBox.BoundsRect.Bottom;
  Inc(Result.cy, BorderWidths[bBottom]);
end;
...
var
  I: Integer;
  AListItemIndex: Integer;
begin
  Result := nil;
  for I := 0 to Length(FCheckedFilterValueIndexes) - 1 do
  begin
    AListItemIndex := ListBoxItemIndexByFilterValueIndex[FCheckedFilterValueIndexes[I]];
    if (AListItemIndex <> -1) and IsCheck(AListItemIndex) then
    begin
      SetLength(Result, Length(Result) + 1);
      Result[Length(Result) - 1] := AListItemIndex;
    end;
  end;
end;
...
begin
  Result := ListBoxItemIndexByFilterValueIndex[
    FValueList.GetIndexByCriteriaItem(Item.DataBinding.FilterCriteriaItem)];
end;
...
begin
  if IsButtonVisible then
  begin
    FButton.Left := BorderWidths[bLeft];
    ...
    if IsSearchInfoPanelVisible then
      FButton.Top := ViewInfo.SearchInfoPanelBounds.Bottom
    else
...
procedure TcxGridFilterPopup.InitChildControls;
var
  R: TRect;
begin
  FListBox.Left := BorderWidths[bLeft];
  if UseIncrementalFiltering then
    FIncrementalFilteringHelper.UpdateSearchEditPosition;
  InitListBox;
  if IsSearchInfoPanelVisible then
  begin
    FListBox.Height := ViewInfo.GetSearchInfoPanelHeight;
    FListBox.Width := Max(ListBox.Width, ViewInfo.GetSearchInfoPanelWidth);
    R := FListBox.BoundsRect;
  end
  else
    R := cxNullRect;
  ViewInfo.SearchInfoPanelBounds := R;
  if UseIncrementalFiltering then
    FIncrementalFilteringHelper.SearchEdit.Width := FListBox.Width - cxMarginsWidth(GetSearchEditOffsets);   
  InitButton;
end;
...
begin
  if UseIncrementalFiltering then
    FListBox.Top := FIncrementalFilteringHelper.SearchEdit.BoundsRect.Bottom + GetSearchEditOffsets.Bottom
  else
    FListBox.Top := BorderWidths[bTop];
  AddListBoxItems;
  AdjustListBoxSize;
  ...
  FListBox.Visible := not IsSearchInfoPanelVisible;
end;
...
begin
  ...
  try
    ...
    if UseIncrementalFiltering then
    begin
      FIncrementalFilteringHelper.SearchEdit.Parent := Self;
      ActiveControl := FIncrementalFilteringHelper.SearchEdit;
    end
    else
      FIncrementalFilteringHelper.SearchEdit.Parent := nil;
    ViewInfo.Font := Font;
    Item.DataBinding.GetFilterStrings(FFilterItems, FValueList);
    Item.DataBinding.GetFilterActiveValueIndexes(FValueList, FCheckedFilterValueIndexes);
    InitChildControls;
...
begin
  Result := GetFilterValueByListItemIndex(AItemIndex).Kind in
    [fviBlanks, fviValue, fviSpecial, fviUserEx];
end;

function TcxGridFilterPopup.IsMRUItemsListSeparator(AItemIndex: Integer): Boolean;
begin
  Result := GetFilterValueByListItemIndex(AItemIndex).Kind = fviMRUSeparator;
end;

procedure TcxGridFilterPopup.KeyDown(var Key: Word; Shift: TShiftState);
begin
  inherited KeyDown(Key, Shift);
  if Key = VK_ESCAPE then
  begin
    if UseIncrementalFiltering and (GetSearchText <> '') then
      FIncrementalFilteringHelper.SearchEdit.Clear
    else
      CloseUp;
    Key := 0;
  end;
...
procedure TcxGridFilterPopup.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
  if (Operation = opRemove) and (AComponent = Item) then
    SetItem(nil);
end;

procedure TcxGridFilterPopup.OwnerChanged;
begin
  if Owner <> nil then
    SetItem((Owner as IcxGridFilterPopupOwner).GetItem);
end;
...
begin
  inherited Paint;
  if IsSearchInfoPanelVisible then
    ViewInfo.DrawSearchInfoPanel(Canvas);
end;

procedure TcxGridFilterPopup.ResizePopupWindow;
var
  ASize: TSize;
  APos: TPoint;
begin
...
procedure TcxCustomGridCustomizationPopup.CheckListBoxDragOver(Sender, Source: TObject;
  ...
  begin
    ...
    try
      GetCheckListBoxSelectedItems(AItems);
      Result := (AInsertionIndex <> -1) and CanMoveItems(AItems);
    finally
      AItems.Free;
    end;
  end;
...
begin
  ...
  Accept := (AItemIndex <> -1) and CanDragOver(AItemIndex);
  if not Accept then
    Exit;
...
function TcxCustomGridCustomizationPopup.CanMoveItems(AItems: TList): Boolean;
var
  I: Integer;
begin
  Result := True;
...
begin
  ...
  if APosition.Y < ADesktopWorkArea.Top then
    ADesktopSpace := APosition.Y + ASize.cy - ADesktopWorkArea.Top
  else
  ...
  begin
    ...
    until (ADesktopWorkArea.Top <= APosition.Y - GetScrollBarSize.cy) and
      (APosition.Y + ASize.cy + GetScrollBarSize.cy <= ADesktopWorkArea.Bottom);
...
begin
  ...
  if Controller.FindPanel.Visible then
...
const
  scxDoesntMatchPropertyType = 'The Properties property type of the %s component doesn' +
  ...
  begin
    ...
    try
      ...
      if csAncestor in ComponentState then
        raise Exception.Create(Format(scxDoesntMatchPropertyType, [AItem.Name]))
      else
        raise;
    end;
  end;
...
begin
  Result := inherited GetIsControlFocused or
    ...
    Controller.FindPanel.Visible and Controller.FindPanel.Edit.IsFocused;
end;
...
begin
  ...
  if AInfo is TcxFindFilterChangedInfo then
    Controller.FindPanel.FindFilterChanged;
end;
...
procedure TcxGridFindPanelEditViewInfo.Calculate(ALeftBound: Integer;
  ...
  begin
    if AWidth > cxGridFindPanelEditMaxWidth then
    AWidth := cxGridFindPanelEditMaxWidth
  else
    if AWidth < cxGridFindPanelEditMinWidth then
      AWidth := cxGridFindPanelEditMinWidth;
  end;
...
var
  AEditSizeProperties: TcxEditSizeProperties;
begin
  if Edit.NeedRecalculate then
  begin
    Edit.NeedRecalculate := False;
    AEditSizeProperties.Height := -1;
    AEditSizeProperties.Width := -1;
    AEditSizeProperties.MaxLineCount := 1;
    Result := Edit.Properties.GetEditSize(cxScreenCanvas, Edit.Style,
      False, null, AEditSizeProperties).cy;
    Edit.Height := Result;
  end
  else
    Result := Edit.Height;
...
begin
  Result := cxGridFindPanelEditMaxWidth;
end;
...
begin
  if not Edit.Visible then
    Edit.Show(GridView.Site, Bounds);
end;
...
function TcxGridFindPanelEditViewInfo.GetEdit: TcxGridFindPanelMRUEdit;
begin
  Result := Controller.FindPanel.Edit;
end;
...
begin
  Result := inherited DoCalculateHeight;
  if Result < cxGridFindPanelButtonMinHeight then
    Result := cxGridFindPanelButtonMinHeight;
end;

function TcxGridFindPanelButtonViewInfo.DoCalculateWidth: Integer;
begin
  Result := inherited DoCalculateWidth;
  if Result < cxGridFindPanelButtonMinWidth then
    Result := cxGridFindPanelButtonMinWidth;
end;
...
begin
  ALeftBound := Min(ALeftBound, Container.FilterViewInfo.TextBounds.Right +
    cxGridCellTextOffset + FilterTextOffset + Container.GetButtonsOffset);
  inherited;
end;
...
begin
  ALeftOffset := WidthLeftPart;
  if ALeftOffset = 0 then
    ALeftOffset := GetFirstOffset;
...
begin
  ...
  try
    ...
    if Item.CanFocus(RecordViewInfo.GridRecord) then
      TcxCustomEditViewInfoAccess(FEditViewInfo).CheckCursor(AMousePos);
