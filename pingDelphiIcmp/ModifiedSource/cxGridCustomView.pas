...
type
  ...
  TcxCustomGridPopup = class(TdxCustomIncrementalFilteringPopupWindow)
  private
    FGridView: TcxCustomGridView;
  ...
  protected
    ...
    procedure DrawItemText(ACanvas: TcxCanvas; AIndex: Integer; const ARect: TRect); virtual;
...
procedure TcxCustomGridPopup.OwnerChanged;
begin
end;
...
procedure TcxCustomGridPopup.CorrectBoundsWithDesktopWorkArea(var APosition: TPoint; var ASize: TSize);
var
...
begin
  if not cxPointIsEqual(FPrevMousePos, Point(X, Y)) then
...
begin
  ...
  DrawItemText(ACanvas, AIndex, ARect);
end;

procedure TcxGridPopupListBox.DrawItemText(ACanvas: TcxCanvas; AIndex: Integer;
  const ARect: TRect);
begin
  ACanvas.DrawText(Items[AIndex], ARect, cxSingleLine or cxAlignLeft or cxAlignVCenter);
end;
...
begin
{  if GridView.IsControlFocused then       // focused row on click; dorofeev
...
procedure TcxCustomGridPainter.DrawRoundBorder;
begin
  Canvas.Pen.Color := clGray;
  Canvas.Brush.Style:=bsClear;
  Canvas.Pen.Width:= 1;
  Canvas.RoundRect(ViewInfo.bounds , 16, 16);
end;
...
procedure TcxCustomGridPainter.Paint(ACanvas: TcxCanvas = nil; // dorofeev
...
procedure TcxGridSite.DoPaint;  // dorofeev (actual grid painter)
begin
...
begin
  if not FViewInfo.IsCalculating and GridView.Visible and (IgnoreUpdateLock or not Locked) then
