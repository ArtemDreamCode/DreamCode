...
{                                                                    }
{           Copyright (c) 2000-2016 Developer Express Inc.           }
{           ALL RIGHTS RESERVED                                      }
{                                                                    }
...
uses
  RTLConsts, Math, Graphics, cxGraphics, cxControls, dxHashUtils, dxHash;
...
begin
    Result := cxGenerateComponentName(AOwnerComponent,
      AComponent.ClassName, APrefixName, ASuffixName, 0);
end;
...
begin
  ...
  begin
    ...
    begin
      if Supports(TObject(AForm), IcxDialogMetricsClientSize, IClientSize) then
        IClientSize.SetClientSize(Size(FClientWidth, FClientHeight))
      else
      begin
  ...
  if Supports(TObject(AForm), IcxDialogMetricsInfoData, IData) and (FData <> nil) then
...
begin
  ...
  if Supports(TObject(AForm), IcxDialogMetricsClientSize, IClientSize) then
  begin
    ASize := IClientSize.GetClientSize;
    FClientHeight := ASize.cy;
    FClientWidth := ASize.cx;
  end
  else
  begin
  ...
  FreeCustomData;
  if Supports(TObject(AForm), IcxDialogMetricsInfoData, IData) and (IData.GetInfoDataSize > 0) then
  begin
