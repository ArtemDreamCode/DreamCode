    ...
    <div class = "settings">
        ...
        <div class="setting__devices-button">
            <a v-if="devices_old.length > 0" href = "settings" class="devices-button__item">
                <span class = "button-text">Подключенные устройства</span>
            </a>
            <a href = "/new-devices" class="devices-button__item button--active">
                <div v-if="message__update > 0"   class="message-count">
                    <span class = "message-count__number">{{message__update}}</span>
                </div>
                <span class = "button-text">Новые устройства</span>
            </a>
        </div>
...
export default {
  ...
  mounted() {
		...
		if (localStorage.devices_old) {
			this.devices_old = JSON.parse(localStorage.getItem("devices_old"));
		}
		if (localStorage.devices_new) {
			var devices = JSON.parse(localStorage.getItem("devices_new"));
