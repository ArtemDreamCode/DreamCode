<template>
    <article>
        <div v-for="devicesBox in devices" :key="devicesBox.id" class = "devices__box">
            <div v-for="device in devicesBox" :key="device.id" @click="devices_active(device,$event)" class="devices__item" :class="{'device--active': device.state == 'off' ? false : true}">
...
export default {
    ...
    methods: {
		...
		let dev = item.classList.contains("device--active") ? {"ip": device.ip, "turn": "on"} : {"ip": device.ip, "turn": "off"}
	...
	if (localStorage.devices_old) {
			...
			devices.forEach((device, index, array) => {
				readyArray.push(device)
				if(readyArray.length == 4 || index + 1 == array.length) {
					this.devices.push(readyArray);
					readyArray = [];
			} 
		})
		}
...
.devices__item:nth-child(1n) {
    grid-area : col1;
}
.devices__item:nth-child(2n) {
    grid-area : col2;
}
.devices__item:nth-child(3n) {
    grid-area : col3;
}
.devices__item:nth-child(4n) {
    grid-area : col4;
}
