<template>
    <article>
        <div v-for="devicesBox in devices" :key="devicesBox.id" class = "devices__box">
            <div v-for="device in devicesBox" :key="device.id" @click="devices_active(device)" :id="device.ip" class="devices__item" :class="{'device--active': device.state == 'off' ? false : true}">
...
export default {
    ...
    methods: {
        devices_active (device) {
			socket.emit("relay", {"ip":device.ip, "turn": device.state == "off" ? "on" : "off"})
		}
    },
	mounted() {
